...
Install directly from GitHub using straight.el:

#+begin_src emacs-lisp
  (use-package dired-narrow
    :straight (:host github :repo "Fuco1/dired-hacks" :files ("dired-narrow.el"))
    :after dired
    :bind (:map dired-mode-map
                ("/" . dired-narrow)))
#+end_src

** Quick preview and close with q
#+begin_src emacs-lisp
  ;; Split vertically when opening files in dired with 'o'
  (setq split-height-threshold nil)
  (setq split-width-threshold 0)

  (defun my/dired-preview-close-window ()
    "Close the current buffer and window, useful for files opened from dired with 'o'."
    (interactive)
    (kill-buffer)
    (delete-window))

  (defun my/setup-preview-close-keybinding ()
    "Set up 'q' to close buffer and window in files opened from dired."
    (local-set-key (kbd "q") #'my/dired-preview-close-window))

  ;; Apply to common file modes
  (add-hook 'text-mode-hook #'my/setup-preview-close-keybinding)
  (add-hook 'prog-mode-hook #'my/setup-preview-close-keybinding)
#+end_src
...
#+begin_src emacs-lisp
    ...
    (defun my/dired-copy-to-rihome ()
      "Copy the file or directory at point to ~/rihome/."
      (interactive)
      (let* ((source (dired-get-filename))
             (target-dir (expand-file-name "~/rihome/"))
    ...
    (with-eval-after-load 'dired
      ...
      ;; Copy commands: C for compress, c c for copy, c r for copy to rihome
      (define-key dired-mode-map (kbd "c") nil)  ; Unbind c first to make it a prefix
      (define-key dired-mode-map (kbd "C") 'dired-do-compress-to)
      (define-key dired-mode-map (kbd "c c") 'dired-do-copy)
      (define-key dired-mode-map (kbd "c r") 'my/dired-copy-to-rihome)

      ;; Navigation: b for back (up directory)
